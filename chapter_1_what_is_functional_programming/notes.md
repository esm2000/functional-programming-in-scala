...
* Functional programming refers to programming with pure functions
    ...
    - Referential transparency - For an expression to be considered referential transparent, the expression can be replaced by its result without changing the meaning or result of the program. When expressions are referentially tranpsarent, we can image that computation proceeds much like the wya we'd solve an alegraic expression.
...
* Functional programming is more modular
    = easier to test, reuse, parallelize, generalize and reaosn about